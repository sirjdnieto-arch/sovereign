...
function renderDeepTechKV(d) {
  ...
  const items = [
    ...
    ["Vol días cero",    String(t.volume_zero_days??"—")],
...
porfa, escribe el código entero

claude-sonnet-4-6-search
No Sources
El problema tiene dos causas y necesita dos fixes:

Python: make_ticker_detail_json devuelve None si el df tiene pocos datos → no se guarda el JSON → el frontend explota. Hay que guardar siempre un JSON, aunque sea sin datos de gráfico.
Frontend: cuando el fetch falla (404), hay que caer en los datos que ya tenemos en all_assets.json en vez de mostrar un error vacío.
Fix Python — make_ticker_detail_json nunca devuelve None
...
def make_ticker_detail_json(ticker, tech_result, fund_result=None):
    """
    Genera el JSON para el módulo Deep Dive.
    Siempre devuelve un dict (nunca None).
    Si no hay datos de precio suficientes, devuelve un esqueleto
    con los datos del snapshot técnico y sin series de gráfico.
    """
    # ── Esqueleto mínimo garantizado ──────────────────────
    skeleton = {
        "ticker":         ticker,
    ...
    try:
        ...
        min_bars = CONFIG["PVI_MA"] + 10   # 130 velas mínimo

        if df.empty:
            skeleton["chart_error"] = "yfinance no devolvió datos para este ticker."
            return skeleton

        if len(df) < min_bars:
            skeleton["chart_error"] = (
                f"Histórico insuficiente: {len(df)} velas disponibles "
                f"(mínimo {min_bars})."
            )
            return skeleton
    ...
    def bool_series_to_markers(s, series_type, color, shape, position):
        out = []
        for t, v in s.items():
            if bool(v):
                out.append({
    ...
    # ── Marcadores ────────────────────────────────────────
    markers = (
        bool_series_to_markers(
            df["PVI_Cross_Up"],   "buy",      "#22c55e", "arrowUp",   "belowBar") +
        bool_series_to_markers(
            df["PVI_Cross_Down"], "sell_pvi", "#ef4444", "arrowDown", "aboveBar") +
        bool_series_to_markers(
            df["McG_Cross_Down"], "sell_mcg", "#f97316", "arrowDown", "aboveBar")
    )
    markers.sort(key=lambda x: x["time"])

    # ── Distancia diaria al McGinley de salida (%) ────────
    dist_series = (
        (df["Close"] - df["McG_Exit"])
        / df["McG_Exit"].replace(0, np.nan)
        * 100
    ).round(2)
...
Fix save_ticker_detail en main() — loguear el motivo
Python
    ...
    def save_ticker_detail(row):
        ...
        try:
            ...
            detail = make_ticker_detail_json(t, tech, fund)   # siempre devuelve dict
            ...
            if not detail["has_chart_data"]:
                print(f"   ⚠️  {t}: JSON guardado sin gráfico → {detail['chart_error']}")

            return t

        except Exception as e:
            print(f"   ❌ Deep Dive {t}: {e}")
            return None
Fix Frontend — loadDeepDive con fallback a allAssets
Reemplaza solo la función loadDeepDive en el JS:
...
async function loadDeepDive() {
  ...
  // ── Intento 1: JSON de ticker completo ────────────────
  try {
    ...
    // fetch error de red — continuamos al fallback
  }

  // ── Intento 2: fallback desde all_assets.json ─────────
  if (!data) {
    const asset = allAssets.find(
      a => String(a.ticker).toUpperCase() === ticker
    );
    ...
    if (!asset) {
      loading.innerHTML = `
        <div style="padding:16px;background:rgba(239,68,68,.1);
          ...
          ❌ <b>${ticker}</b> no está en el universo analizado.<br>
          <span class="small">Comprueba que el ticker es correcto y está en UNIVERSE.</span>
        </div>`;
      return;
    }

    // Construir data sintética desde el snapshot de allAssets
    data = {
      ticker:         asset.ticker,
      ...
      has_chart_data: false,
      chart_error:    "JSON de gráfico no generado (datos insuficientes o ticker problemático).",
      candles:        [],
      mcg_regime:     [],
      mcg_exit:       [],
      pvi:            [],
      ...
      markers:        [],
      technical:      asset,
      fundamental:    asset.has_fundamentals ? asset : null,
    };
  }
  ...
  renderDeepCharts(data);     // ← función unificada que decide qué mostrar
  renderDeepTechKV(data);
  renderDeepFundamentals(data);
}
Fix Frontend — renderDeepCharts unificada
Reemplaza las 4 funciones renderDeepChart* por esta única función que decide si dibujar o mostrar el aviso:

JavaScript

function renderDeepCharts(d) {
  // ── Sin datos de gráfico: mostrar aviso en cada panel ──
  if (!d.has_chart_data) {
    const msg = `
      <div style="display:flex;align-items:center;justify-content:center;
           height:100%;min-height:80px;color:var(--muted);font-size:13px;
           background:rgba(30,41,59,.4);border-radius:10px;padding:18px;
           border:1px dashed var(--border)">
        ⚠️ ${d.chart_error || "Sin datos de gráfico disponibles"}
      </div>`;

    ["dd-chart-price","dd-chart-pvi","dd-chart-rvol","dd-chart-dist"]
      .forEach(id => {
  ...
  // ── Con datos: renderizar los 4 gráficos normales ──────
  renderDeepChartPrice(d);
  renderDeepChartPVI(d);
  renderDeepChartRVOL(d);
  renderDeepChartDist(d);
}
Las funciones renderDeepChartPrice, renderDeepChartPVI, renderDeepChartRVOL y renderDeepChartDist no cambian, solo se llaman ahora desde renderDeepCharts.

Qué soluciona cada cambio
Problema	Fix
MOMO (y cualquier ticker con pocos datos) no genera JSON → 404	make_ticker_detail_json siempre guarda el archivo, con has_chart_data: false y el motivo en chart_error
El frontend explota con 404	loadDeepDive hace fallback a allAssets y construye un data sintético
Ticker completamente fuera del universo	Mensaje de error claro con instrucciones
Gráficos vacíos sin explicación	renderDeepCharts muestra el chart_error en cada panel en lugar de un chart vacío
Log de main() silencioso	save_ticker_detail imprime el motivo exacto por el que un ticker no tiene gráfico




No se encontraron datos para "MOMO". Asegúrate de que está en el universo y se ha generado el JSON.

claude-sonnet-4-6-search
No Sources
Módulo "🔍 Deep Dive" — Análisis Profundo por Ticker
La solución añade:

Python: función make_ticker_detail_json() que guarda data/tickers/TICKER.json con el histórico completo + indicadores
Frontend: nueva pestaña con chart interactivo via lightweight-charts (TradingView, CDN gratuito), sub-gráficos de PVI y RVOL/ATR, y todas las métricas fundamentales en cards
Cambios en Python
1. Modifica analyze_technical para devolver también el df
Busca la función analyze_technical y cambia la línea del return final (el gran dict al final) para que además devuelva el dataframe. Lo más limpio es añadir una función separada:

Python

# ============================================================
# DEEP DIVE — Generador de datos por ticker
# ============================================================

def make_ticker_detail_json(ticker, tech_result, fund_result=None):
    """
    Genera el JSON completo para el módulo Deep Dive de un ticker.
    Incluye velas OHLCV, todos los indicadores y los fundamentales.
    Se guarda en data/tickers/{TICKER}.json
    """
    df = yf_download_prices(ticker, CONFIG["PRICE_PERIOD"])
    ...
    for t, row in df.iterrows():
        candles.append({
            "time":   to_time(t),
            "open":   round(safe_float(row["Open"]),   4),
            "high":   round(safe_float(row["High"]),   4),
            "low":    round(safe_float(row["Low"]),    4),
            "close":  round(safe_float(row["Close"]),  4),
            "volume": int(safe_float(row["Volume"])) if np.isfinite(safe_float(row["Volume"])) else 0,
        })

    # Marcadores de señal (para superponer en el gráfico de precio)
    markers = (
        bool_series_to_markers(df["PVI_Cross_Up"],   "buy",      "#22c55e", "arrowUp",   "belowBar") +
        bool_series_to_markers(df["PVI_Cross_Down"],  "sell_pvi", "#ef4444", "arrowDown", "aboveBar") +
        bool_series_to_markers(df["McG_Cross_Down"],  "sell_mcg", "#f97316", "arrowDown", "aboveBar")
    )
    markers.sort(key=lambda x: x["time"])

    # Calcular distancia diaria al McGinley de salida (%)
    dist_series = ((df["Close"] - df["McG_Exit"]) / df["McG_Exit"].replace(0, np.nan) * 100).round(2)
    ...
    detail = {
        ...
        # ── Snapshot técnico (resultado de analyze_technical) ─
        "technical": {k: clean_json_value(v) for k, v in tech_result.items()
                      if k not in ("error",)},
    ...
    return detail
2. Modifica main() para guardar los JSONs de ticker
Añade esto dentro de main(), justo después de assets_df = add_fundamentals(...) y antes de generar signals_df:

Python

    # ── Generar JSONs de Deep Dive por ticker ────────────────
    print("🔬 Generando datos Deep Dive por ticker...")
    tickers_dir = data_dir / "tickers"
    tickers_dir.mkdir(parents=True, exist_ok=True)

    # Construir lookup de fundamentales
    fund_lookup = {}
    ...
    def save_ticker_detail(row):
        ...
        try:
            ...
            print(f"   ⚠️ Deep Dive {t}: {e}")
        return None

    with ThreadPoolExecutor(max_workers=CONFIG["MAX_WORKERS"]) as executor:
        futures_dd = {executor.submit(save_ticker_detail, row): row["ticker"]
                      for _, row in assets_df.iterrows()}
        saved = 0
        for fut in as_completed(futures_dd):
            if fut.result():
                saved += 1

    print(f"   ✅ {saved} JSONs de ticker guardados en /site/data/tickers/")
Cambios en el Frontend (INDEX_HTML)
Reemplaza el INDEX_HTML completo con la versión siguiente. Los únicos cambios son:

Nueva pestaña 🔍 Deep Dive
Nueva <section id="deepdive">
Carga de lightweight-charts en el <head>
Nuevas funciones JS loadDeepDive, renderDeepChart, renderDeepFundamentals, etc.
Python
...
"""
Resumen de lo que hace el módulo
Elemento	Detalle
Selector de ticker	Input libre + 40 atajos rápidos (con señal activa resaltados en azul)
Gráfico de precio	Velas OHLCV + volumen en fondo + McGinley Régimen (azul) + McGinley Salida (naranja) + marcadores ▲▼ de todos los cruces
Gráfico PVI	PVI (azul) vs señal EMA-120 (ámbar) + marcadores de cruce
Gráfico RVOL	Histograma con colores (verde ≥1.5x, gris <1.5x) + líneas de umbral 1.5x y 1.0x
Gráfico distancia McG	Histograma verde/rojo de (close/mcg_exit - 1) %
8 tarjetas de señal	Señal, Frescura, Calidad entrada, Oportunidad global, Presión salida, Stop, PVI estado, RVOL/CHOP/ATR
25 KV técnicos	Todos los valores snapshot del analyze_technical
Fundamentales completos	6 sub-scores con barras, P&L TTM, Balance, Márgenes, Múltiplos, Piotroski/Altman/Beneish con semáforos automáticos




me gustaria crear un módulo dónde selecciones un ticker y podemos tener un análisis más profundo, interactivo. Por ejemplo, poder ver el gráfico con el precio, los indicadores , el cruce, el numero de velas de cruce, atr,diatancia a mcgingle + fundamentales de yfinance...bueno organízalo como veas, se trata de ver todo mas en profucndidad, con datos que ya tenemos. El código al que incluirlo es:
...
<!doctype html><html lang="es"> <head> <meta charset="utf-8"> <meta name="viewport" content="width=device-width, initial-scale=1"> <title>LCrack Sovereign</title><style> :root { --bg: #020617; --panel: #0f172a; --border: rgba(148,163,184,.22); --text: #e5e7eb; --muted: #94a3b8; --green: #86efac; --red: #fca5a5; --yellow: #fde68a; --blue: #93c5fd; --purple: #d8b4fe; --orange: #fdba74; } * { box-sizing: border-box; } body { margin: 0; font-family: Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif; background: radial-gradient(circle at top left, rgba(30,64,175,.42), transparent 28%), radial-gradient(circle at bottom right, rgba(7,89,133,.24), transparent 28%), var(--bg); color: var(--text); } .container { max-width: 1650px; margin: auto; padding: 28px; } h1 { font-size: 38px; margin: 0; letter-spacing: -.03em; } .subtitle { color: var(--muted); margin-top: 8px; margin-bottom: 24px; } .grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; margin-bottom: 18px; } .grid2 { display: grid; grid-template-columns: 1.1fr .9fr; gap: 14px; margin-bottom: 18px; } .card { background: rgba(15, 23, 42, .92); border: 1px solid var(--border); border-radius: 18px; padding: 18px; box-shadow: 0 10px 35px rgba(0,0,0,.25); } .metric-title { color: var(--muted); font-size: 13px; font-weight: 800; text-transform: uppercase; letter-spacing: .05em; } .metric-big { font-size: 28px; font-weight: 950; margin-top: 8px; } .small { font-size: 12px; color: var(--muted); margin-top: 5px; line-height: 1.35; } .tabs { display: flex; flex-wrap: wrap; gap: 8px; margin: 22px 0; } .tab { cursor: pointer; border: 1px solid var(--border); background: rgba(15,23,42,.72); color: var(--text); border-radius: 999px; padding: 10px 14px; font-weight: 800; } .tab.active { background: rgba(59,130,246,.25); color: var(--blue); border-color: rgba(59,130,246,.5); } .section { display: none; } .section.active { display: block; } table { width: 100%; border-collapse: collapse; overflow: hidden; border-radius: 14px; border: 1px solid var(--border); } th { background: rgba(30,41,59,.96); color: #cbd5e1; text-align: left; padding: 11px; font-size: 12px; text-transform: uppercase; letter-spacing: .04em; position: sticky; top: 0; } td { border-top: 1px solid rgba(148,163,184,.13); padding: 11px; vertical-align: top; } tr:hover { background: rgba(30,41,59,.5); } .badge { display: inline-block; padding: 5px 9px; border-radius: 999px; font-size: 12px; font-weight: 900; border: 1px solid var(--border); white-space: nowrap; margin: 1px; } .buy { color: var(--green); background: rgba(34,197,94,.15); border-color: rgba(34,197,94,.35); } .sell { color: var(--red); background: rgba(239,68,68,.15); border-color: rgba(239,68,68,.35); } .partial { color: var(--yellow); background: rgba(245,158,11,.15); border-color: rgba(245,158,11,.35); } .neutral { color: #cbd5e1; background: rgba(148,163,184,.12); border-color: rgba(148,163,184,.28); } .mixed { color: var(--purple); background: rgba(168,85,247,.15); border-color: rgba(168,85,247,.35); } .qbadge { color: var(--blue); background: rgba(59,130,246,.15); border-color: rgba(59,130,246,.35); } .vbadge { color: var(--orange); background: rgba(251,146,60,.15); border-color: rgba(251,146,60,.35); } .warnbadge { color: var(--yellow); background: rgba(245,158,11,.14); border-color: rgba(245,158,11,.35); } .ticker { font-weight: 950; font-size: 16px; } .name { color: var(--muted); font-size: 12px; margin-top: 3px; } .controls { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 14px; align-items: center; } input, select, button { border: 1px solid var(--border); background: rgba(15,23,42,.95); color: var(--text); border-radius: 10px; padding: 10px; } button { cursor: pointer; font-weight: 800; } button.primary { background: rgba(37,99,235,.42); border-color: rgba(59,130,246,.55); color: white; } button.danger { background: rgba(220,38,38,.22); border-color: rgba(248,113,113,.4); color: var(--red); } .form-grid { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr 2fr auto; gap: 8px; margin-bottom: 14px; } .warning { color: var(--yellow); } .score-bar-wrap { width: 100%; height: 6px; background: rgba(148,163,184,.2); border-radius: 999px; overflow: hidden; margin-top: 5px; } .score-bar { height: 6px; border-radius: 999px; } .footer { color: var(--muted); margin-top: 28px; font-size: 12px; } @media (max-width: 900px) { .grid, .grid2, .form-grid { grid-template-columns: 1fr; } } </style></head><body> <div class="container"> <h1>🛰️ LCrack Sovereign</h1> <div class="subtitle" id="subtitle">Cargando datos...</div> <div class="grid" id="summaryCards"></div> <div class="tabs"> <button class="tab active" onclick="showTab('global', event)">🌍 Panel global</button> <button class="tab" onclick="showTab('signals', event)">🎯 Señales</button> <button class="tab" onclick="showTab('universe', event)">📡 Universo</button> <button class="tab" onclick="showTab('portfolio', event)">💼 Mi cartera</button> <button class="tab" onclick="showTab('rules', event)">📘 Reglas</button> </div> <section id="global" class="section active"> <div class="grid2"> <div class="card"> <h2>🌍 Macro global</h2> <div id="macroTable"></div> </div> <div class="card"> <h2>💶 EUR/USD para inversor europeo</h2> <div id="fxBox"></div> </div> </div> </section> <section id="signals" class="section"> <div class="card"> <h2>🎯 Señales recientes</h2> <div class="controls"> <input id="signalSearch" placeholder="Buscar ticker o nombre..." oninput="renderSignals()"> <select id="signalType" onchange="renderSignals()"> <option value="">Todas las señales</option> <option value="COMPRA">Compras</option> <option value="VENTA">Ventas</option> <option value="MIXTA">Mixtas</option> </select> <select id="signalRegime" onchange="renderSignals()"> <option value="">Todos los regímenes</option> <option value="ALCISTA">Alcista</option> <option value="LATERAL">Lateral</option> <option value="BAJISTA">Bajista</option> </select> </div> <div id="signalsTable"></div> </div> </section> <section id="universe" class="section"> <div class="card"> <h2>📡 Universo completo</h2> <div class="controls"> <input id="universeSearch" placeholder="Buscar ticker, nombre, sector..." oninput="renderUniverse()">
text
...
</section> <section id="portfolio" class="section"> <div class="card"> <h2>💼 Mi cartera privada</h2> <p class="small"> Tus posiciones se guardan solo en este navegador mediante localStorage. </p>
text
...
</section> <!-- ═══════════════════ REGLAS ════════════════════ --> <section id="rules" class="section"> <div class="card"> <h2>📘 Reglas del sistema</h2>
text
...
</section> <div class="footer"> LCrack Sovereign · Datos vía yfinance/FRED/Fear & Greed. </div> </div><script> let allAssets = []; let signals = []; let summary = {}; let portfolio = JSON.parse(localStorage.getItem("sovereign_portfolio") || "[]"); function fmtNum(x, d=2) { if (x === null || x === undefined || Number.isNaN(Number(x))) return "—"; return Number(x).toLocaleString("es-ES", {maximumFractionDigits:d, minimumFractionDigits:d}); } function fmtPct(x, d=1) { if (x === null || x === undefined || Number.isNaN(Number(x))) return "—"; return (Number(x) * 100).toLocaleString("es-ES", {maximumFractionDigits:d, minimumFractionDigits:d}) + "%"; } function clsFor(text) { text = String(text || ""); if (text.includes("COMPRA") || text.includes("ALCISTA") || text.includes("MANTENER") || text.includes("Baja")) return "buy"; if (text.includes("VENTA 100") || text.includes("BAJISTA") || text.includes("Salida fuerte") || text.includes("VENDER TODO")) return "sell"; if (text.includes("VENTA") || text.includes("LATERAL") || text.includes("REDUCIR") || text.includes("Reducir") || text.includes("STOP") || text.includes("Vigilar")) return "partial"; if (text.includes("MIXTA")) return "mixed"; return "neutral"; } function badge(text, cls="") { return `<span class="badge ${cls || clsFor(text)}">${text || "—"}</span>`; } function scoreBar(val, color="#3b82f6") { if (val === null || val === undefined || Number.isNaN(Number(val))) return ""; const pct = Math.max(0, Math.min(100, Number(val))); return `<div class="score-bar-wrap"><div class="score-bar" style="width:${pct}%;background:${color}"></div></div>`; } function qualityColor(score) { if (score === null || score === undefined || Number.isNaN(Number(score))) return "#64748b"; if (score >= 85) return "#86efac"; if (score >= 75) return "#4ade80"; if (score >= 65) return "#a3e635"; if (score >= 50) return "#fde68a"; return "#fca5a5"; } function valuationColor(score) { if (score === null || score === undefined || Number.isNaN(Number(score))) return "#64748b"; if (score >= 80) return "#86efac"; if (score >= 65) return "#4ade80"; if (score >= 45) return "#fde68a"; if (score >= 25) return "#fb923c"; return "#fca5a5"; } function showTab(id, ev) { document.querySelectorAll(".section").forEach(s => s.classList.remove("active")); document.querySelectorAll(".tab").forEach(t => t.classList.remove("active")); document.getElementById(id).classList.add("active"); if (ev && ev.target) ev.target.classList.add("active"); } async function loadData() { allAssets = await fetch("data/all_assets.json").then(r => r.json()); signals = await fetch("data/signals.json").then(r => r.json()); summary = await fetch("data/summary.json").then(r => r.json()); renderSummary(); renderGlobal(); renderSignals(); renderUniverse(); renderPortfolio(); } function renderSummary() { document.getElementById("subtitle").innerText = `Última actualización: ${summary.generated_at || "—"} · Activos analizados: ${summary.total_assets || 0}`; const cards = [ ["🌍 Macro", summary.macro?.label || "—", `${summary.macro?.warning_text || "Score: " + fmtNum(summary.macro?.score, 1)}`], ["💶 EUR/USD", summary.fx?.label || "—", summary.fx?.diag || "—"], ["🎯 Señales", String(summary.total_signals || 0), `Compras: ${summary.buy_signals || 0} · Ventas: ${summary.sell_signals || 0}`], ["⚙️ Parámetros", `${summary.config?.LOOKBACK_SIGNAL || 5} velas`, `PVI ${summary.config?.PVI_MA || 120} · RVOL ${summary.config?.RVOL_HIGH || 1.5}x`], ]; document.getElementById("summaryCards").innerHTML = cards.map(c => ` <div class="card"> <div class="metric-title">${c[0]}</div> <div class="metric-big">${c[1]}</div> <div class="small">${c[2]}</div> </div> `).join(""); } function renderGlobal() { const rows = (summary.macro?.rows || []).map(r => ` <tr> <td>${r.id}</td> <td>${fmtNum(r.value, 2)}</td> <td>${fmtPct(r.roc5, 1)}</td> <td>${fmtPct(r.roc20, 1)}</td> <td>${fmtNum(r.impact, 1)}</td> <td>${r.diag || "—"}</td> </tr> `).join(""); document.getElementById("macroTable").innerHTML = ` <table> <thead> <tr> <th>Indicador</th><th>Valor</th><th>ROC 5D</th><th>ROC 20D</th><th>Impacto</th><th>Diagnóstico</th> </tr> </thead> <tbody>${rows}</tbody> </table> `; const fx = summary.fx || {}; document.getElementById("fxBox").innerHTML = ` <p>${badge(fx.label || "—")}</p> <p>${fx.diag || "—"}</p> <table> <tbody> <tr><td>EUR/USD</td><td>${fmtNum(fx.eurusd, 4)}</td></tr> <tr><td>ROC 5D</td><td>${fmtPct(fx.roc5, 2)}</td></tr> <tr><td>ROC 20D</td><td>${fmtPct(fx.roc20, 2)}</td></tr> <tr><td>SMA 50</td><td>${fmtNum(fx.sma50, 4)}</td></tr> <tr><td>Score</td><td>${fmtNum(fx.score, 1)}</td></tr> </tbody> </table> `; } function fundBlock(a) { const hasFund = a.has_fundamentals === true; if (!hasFund) { return `<div class="small">Sin fundamentales / técnico-only / datos insuficientes</div>`; } return ` <div> ${badge("Calidad " + fmtNum(a.quality_score,0) + " · " + (a.quality_label || "—"), "qbadge")} ${badge("Precio/Fund. " + fmtNum(a.valuation_score,0) + " · " + (a.valuation_label || "—"), "vbadge")} ${badge("Conf " + (a.confidence_grade || "—"), "neutral")} ${badge(a.quality_q || "—", "qbadge")} </div> ${scoreBar(a.quality_score, qualityColor(a.quality_score))} ${scoreBar(a.valuation_score, valuationColor(a.valuation_score))} <div class="small">${a.fundamental_profile || "—"} · ${a.fundamental_trend || "—"}</div> <div class="small"> Calidad: ${fmtNum(a.score_quality,0)} op · ${fmtNum(a.score_cash,0)} caja · ${fmtNum(a.score_solvency,0)} solv · ${fmtNum(a.score_growth,0)} crec · ${fmtNum(a.score_risk,0)} riesgo </div> <div class="small"> ROIC ${fmtPct(a.roic,1)} · ROE ${fmtPct(a.roe,1)} · FCF Yield ${fmtPct(a.fcf_yield,1)} · Altman ${fmtNum(a.altman_z,2)} · Piotroski ${fmtNum(a.piotroski,1)} </div> <div class="small"> P/E ${fmtNum(a.pe,1)} · P/B ${fmtNum(a.pb,2)} · EV/EBITDA ${fmtNum(a.ev_ebitda,1)} · ND/EBITDA ${fmtNum(a.net_debt_ebitda,2)} </div> <div class="small"> ${a.fundamental_model || "—"} · ${a.period_label || "N/A"} · ${a.valuation_style || "—"} </div> ${a.red_flags ? `<div class="small warning">⚠️ ${a.red_flags}</div>` : ""} `; } function techBlock(a) { const macroWarn = summary.macro?.warning && String(a.main_signal || "").includes("COMPRA") ? badge("⚠️ Macro cautelosa", "warnbadge") : ""; const volBlocked = a.raw_buy_blocked_by_vol ? `<div class="small warning">⚠️ Señal de compra bloqueada por volumen insuficiente</div>` : ""; return ` <div> ${badge(a.technical_state || "—")} ${badge(a.signal_freshness || "—", "neutral")} ${macroWarn} </div> <div class="small"> <b>Entrada técnica:</b> ${a.entry_quality_label || "—"} ${a.entry_quality_score !== null && a.entry_quality_score !== undefined ? " · " + fmtNum(a.entry_quality_score,0) + "/100" : ""} </div> <div class="small"> <b>Oportunidad global:</b> ${a.opportunity_label || "—"} ${a.opportunity_score !== null && a.opportunity_score !== undefined ? " · " + fmtNum(a.opportunity_score,0) + "/100" : ""} </div> <div class="small"> ${a.entry_quality_notes || ""} </div> <div class="small"> Salida: <b>${a.exit_pressure_label || "—"}</b> · ${fmtNum(a.exit_pressure_score,0)}/100 </div> <div class="small"> ${a.exit_pressure_notes || ""} </div> <div class="small"> PVI ${a.pvi_status || "—"} · Gap ${fmtPct(a.pvi_gap,2)} · RVOL ${fmtNum(a.rvol,2)}x ${a.bullish_high_volume ? " · 🟢 Vol. alcista alto" : ""} ${a.bearish_high_volume ? " · 🔴 Vol. bajista alto" : ""} </div> <div class="small"> Dist McG ${fmtPct(a.dist_to_mcg_exit,1)} · ATR ${fmtNum(a.dist_to_mcg_exit_atr,2)}x · CHOP ${fmtNum(a.chop,1)}<br> Stop: ${a.stop_status || "—"} </div> ${a.volume_quality === false ? `<div class="small warning">⚠️ ${a.volume_warning || "Volumen dudoso"}</div>` : ""} ${volBlocked} `; } function signalRow(a) { return ` <tr> <td> <div class="ticker">${a.ticker}</div> <div class="name">${a.name || "—"}</div> </td> <td>${badge(a.main_signal)}<div class="small">${a.events_text || ""}</div></td> <td>${badge(a.regime)}<div class="small">Cruces: ${a.recent_crosses ?? "—"}</div></td> <td>${techBlock(a)}</td> <td>${fundBlock(a)}</td> <td>${a.bucket || "—"}</td> </tr> `; } function renderSignals() { const q = (document.getElementById("signalSearch")?.value || "").toUpperCase(); const type = document.getElementById("signalType")?.value || ""; const regime = document.getElementById("signalRegime")?.value || ""; let data = signals.slice(); if (q) { data = data.filter(a => String(a.ticker).toUpperCase().includes(q) || String(a.name).toUpperCase().includes(q) ); } if (type) { data = data.filter(a => String(a.main_signal || "").includes(type)); } if (regime) { data = data.filter(a => String(a.regime || "").includes(regime)); } if (!data.length) { document.getElementById("signalsTable").innerHTML = `<p class="small">No hay señales con esos filtros.</p>`; return; } document.getElementById("signalsTable").innerHTML = ` <table> <thead> <tr> <th>Activo</th><th>Señal</th><th>Régimen</th><th>Técnico avanzado</th><th>Fundamental</th><th>Universo</th> </tr> </thead> <tbody>${data.map(signalRow).join("")}</tbody> </table> `; } function confidenceValue(c) { if (c === "A") return 3; if (c === "B") return 2; if (c === "C") return 1; return 0; } function renderUniverse() { const q = (document.getElementById("universeSearch")?.value || "").toUpperCase(); const regime = document.getElementById("universeRegime")?.value || ""; const confFilter = document.getElementById("fundConfidenceFilter")?.value || ""; const qualMin = document.getElementById("fundQualityFilter")?.value; const valMin = document.getElementById("fundValuationFilter")?.value; const entryMin = document.getElementById("entryQualityFilter")?.value; const exitMin = document.getElementById("exitPressureFilter")?.value; const qFilter = document.getElementById("fundQFilter")?.value || ""; const onlyFund = document.getElementById("onlyFundamentals")?.checked || false; let data = allAssets.slice(); if (q) { data = data.filter(a => String(a.ticker).toUpperCase().includes(q) || String(a.name).toUpperCase().includes(q) || String(a.sector || "").toUpperCase().includes(q) || String(a.industry || "").toUpperCase().includes(q) ); } if (regime) data = data.filter(a => String(a.regime || "").includes(regime)); if (onlyFund) data = data.filter(a => a.has_fundamentals === true); if (confFilter) data = data.filter(a => confidenceValue(a.confidence_grade) >= confidenceValue(confFilter)); if (qualMin) data = data.filter(a => Number(a.quality_score) >= Number(qualMin)); if (valMin) data = data.filter(a => Number(a.valuation_score) >= Number(valMin)); if (entryMin) data = data.filter(a => Number(a.entry_quality_score) >= Number(entryMin)); if (exitMin) data = data.filter(a => Number(a.exit_pressure_score) >= Number(exitMin)); if (qFilter) data = data.filter(a => String(a.quality_q || "") === qFilter); data = data.sort((a,b) => { const fa = a.quality_score === null || a.quality_score === undefined ? -1 : Number(a.quality_score); const fb = b.quality_score === null || b.quality_score === undefined ? -1 : Number(b.quality_score); if (fb !== fa) return fb - fa; return String(a.ticker).localeCompare(String(b.ticker)); }); const rows = data.map(a => ` <tr> <td> <div class="ticker">${a.ticker}</div> <div class="name">${a.name || "—"}</div> </td> <td>${badge(a.main_signal || "—")}</td> <td>${badge(a.regime || "—")}<div class="small">${a.technical_state || "—"}</div></td> <td>${fmtNum(a.close,2)}</td> <td>${techBlock(a)}</td> <td>${fundBlock(a)}</td> <td>${a.bucket || "—"}</td> </tr> `).join(""); document.getElementById("universeTable").innerHTML = ` <table> <thead> <tr> <th>Activo</th> <th>Señal</th> <th>Régimen</th> <th>Precio</th> <th>Técnico avanzado</th> <th>Fundamental</th> <th>Universo</th> </tr> </thead> <tbody>${rows}</tbody> </table> `; } function findAsset(ticker) { return allAssets.find(a => String(a.ticker).toUpperCase() === String(ticker).toUpperCase()); } function savePortfolio() { localStorage.setItem("sovereign_portfolio", JSON.stringify(portfolio)); } function addPosition() { const ticker = document.getElementById("pfTicker").value.trim().toUpperCase(); const quantity = parseFloat(document.getElementById("pfQty").value); const buyPrice = parseFloat(document.getElementById("pfPrice").value); const buyDate = document.getElementById("pfDate").value; const note = document.getElementById("pfNote").value; if (!ticker || !quantity || !buyPrice) { alert("Ticker, cantidad y precio de compra son obligatorios."); return; } const asset = findAsset(ticker); portfolio.push({ ticker, name: asset ? asset.name : ticker, quantity, buy_price: buyPrice, buy_date: buyDate, note }); savePortfolio(); document.getElementById("pfTicker").value = ""; document.getElementById("pfQty").value = ""; document.getElementById("pfPrice").value = ""; document.getElementById("pfNote").value = ""; renderPortfolio(); } function deletePosition(i) { portfolio.splice(i, 1); savePortfolio(); renderPortfolio(); } function clearPortfolio() { if (!confirm("¿Seguro que quieres vaciar la cartera local?")) return; portfolio = []; savePortfolio(); renderPortfolio(); } function systemAction(asset) { if (!asset) { return ["⚪ SIN DATOS", "No hay datos diarios para este activo."]; } const pressure = Number(asset.exit_pressure_score || 0); const signal = asset.main_signal || ""; const state = asset.technical_state || ""; if (signal.includes("VENTA 100") || pressure >= 75) { return ["🔴 VENDER TODO / REVISAR", "Presión de salida fuerte."]; } if (pressure >= 50) { return ["🟠 REDUCIR", "Presión de salida elevada."]; } if (pressure >= 25) { return ["🟡 VIGILAR", "Hay deterioro técnico parcial."]; } if (state.includes("Alcista confirmado")) { return ["🟢 MANTENER", "Estado técnico favorable."]; } return ["⚪ MANTENER / VIGILAR", "Sin señal operativa clara."]; } function renderPortfolio() { if (!portfolio.length) { document.getElementById("portfolioSummary").innerHTML = ""; document.getElementById("portfolioTable").innerHTML = `<p class="small">Aún no tienes posiciones guardadas.</p>`; return; } let totalValue = 0; let totalCost = 0; const rows = portfolio.map((p, i) => { const asset = findAsset(p.ticker); const price = asset ? Number(asset.close) : null; const qty = Number(p.quantity); const buy = Number(p.buy_price); const value = price ? qty * price : null; const cost = qty * buy; const pnl = value !== null ? value - cost : null; const pnlPct = value !== null && cost ? value / cost - 1 : null; if (value !== null) totalValue += value; totalCost += cost; const [action, reason] = systemAction(asset); return ` <tr> <td> <div class="ticker">${p.ticker}</div> <div class="name">${p.name || (asset ? asset.name : "")}</div> ${asset && asset.has_fundamentals ? `<div class="small">Cal ${fmtNum(asset.quality_score,0)} · Val ${fmtNum(asset.valuation_score,0)} · ${asset.fundamental_trend || "—"}</div>` : ""} ${asset && asset.opportunity_score !== null && asset.opportunity_score !== undefined ? `<div class="small">Oport. global ${fmtNum(asset.opportunity_score,0)} · ${asset.opportunity_label || "—"}</div>` : ""} </td> <td>${fmtNum(qty,4)}</td> <td>${fmtNum(buy,2)}</td> <td>${price ? fmtNum(price,2) : "—"}</td> <td>${pnlPct !== null ? fmtPct(pnlPct,2) : "—"}<div class="small">${pnl !== null ? fmtNum(pnl,2) : "—"}</div></td> <td>${asset ? badge(asset.main_signal || "—") : "—"}<div class="small">${asset ? (asset.events_text || "") : ""}</div></td> <td>${asset ? badge(asset.exit_pressure_label || "—") : "—"}<div class="small">${asset ? asset.exit_pressure_notes || "" : ""}</div></td> <td>${badge(action)}<div class="small">${reason}</div></td> <td>${p.buy_date || "—"}<div class="small">${p.note || ""}</div></td> <td><button class="danger" onclick="deletePosition(${i})">Borrar</button></td> </tr> `; }).join(""); const totalPnl = totalValue - totalCost; const totalPnlPct = totalCost ? totalValue / totalCost - 1 : null; document.getElementById("portfolioSummary").innerHTML = ` <div class="grid"> <div class="card"><div class="metric-title">Valor cartera</div><div class="metric-big">${fmtNum(totalValue,2)}</div></div> <div class="card"><div class="metric-title">Coste</div><div class="metric-big">${fmtNum(totalCost,2)}</div></div> <div class="card"><div class="metric-title">PnL</div><div class="metric-big">${fmtNum(totalPnl,2)}</div><div class="small">${fmtPct(totalPnlPct,2)}</div></div> <div class="card"><div class="metric-title">Posiciones</div><div class="metric-big">${portfolio.length}</div></div> </div> `; document.getElementById("portfolioTable").innerHTML = ` <table> <thead> <tr> <th>Activo</th><th>Cantidad</th><th>Compra</th><th>Actual</th><th>PnL</th> <th>Señal</th><th>Presión salida</th><th>Acción sistema</th><th>Fecha/Nota</th><th></th> </tr> </thead> <tbody>${rows}</tbody> </table> `; } function exportPortfolio() { const blob = new Blob([JSON.stringify(portfolio, null, 2)], {type: "application/json"}); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "sovereign_portfolio.json"; a.click(); } function importPortfolio(event) { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = function(e) { try { portfolio = JSON.parse(e.target.result); savePortfolio(); renderPortfolio(); } catch { alert("No se pudo importar el JSON."); } }; reader.readAsText(file); } loadData(); </script></body> </html> """
============================================================
MAIN
============================================================
def main():
